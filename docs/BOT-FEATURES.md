...
이 문서는 **봇이 채팅에서 무엇을 하는가** 만 다룹니다.
서비스 구조(사용자 · 관리자 · 내부관리자 · Core)는 [ARCHITECTURE.md](ARCHITECTURE.md) 를 보세요.

봇 설정은 채널 관리 대시보드(`/dashboard`)에서 바꾸며, 저장하면 **재시작 없이** 반영됩니다.

봇은 채널마다 하나씩 뜨고, 스트리머나 매니저가 채팅에 `!입장` 을 쳐야 응답을 시작합니다.
`!퇴장` 으로 내보낼 수 있고, 대시보드 헤더의 버튼도 같은 동작을 합니다.
